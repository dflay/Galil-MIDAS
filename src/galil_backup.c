...
{ 
 ...
 myfile <<"#MOVE\nKIA=1\nKPA=103\nKDA=2268\nSPA=8000\nACA=20000\nTLA=8\nKIB=1\nKPB=103\nKDB=2268\nSPB=8000\nACB=20000\nTLB=8\nKIC=1\nKPC=103\nKDC=2268\nSPC=8000\nACC=20000\nTLC=8\n#A\npos=_TPA\npos1=_TPB\npos2=_TPC\nsp=_TVA\nsp1=_TVB\nsp2=_TVC\nacc=_ACA\nacc1=_ACB\nacc2=_ACC\nMG pos, pos1, pos2, sp, sp1, sp2, acc, acc1, acc2\nWT500\nJP#A\nEN\n";
 myfile.close();
...
INT read_galil_event(char *pevent, INT off){
 ...
 hkeyclient=0;
  ...
  iss >> acc2;
  iss >> acc3;
  cout << "positions are " << pos1 << " " << pos2 << " " << pos3 << " " << sp1 <<" " << sp2 << " " << sp3 << endl;
  ...
  speed[1]=(int)sp2;
  speed[2]=(int)sp3;
  acceleration[0]=(int)acc1;
  acceleration[1]=(int)acc2;
  acceleration[2]=(int)acc3;
   
 cm_get_experiment_database(&hDB, NULL);

 db_set_value(hDB, 0, "/Equipment/Galil/Variables/Position",&axes, sizeof(axes), 3, TID_INT);
 db_set_value(hDB,0,"/Equipment/Galil/Variables/Speed",&speed,sizeof(speed),3,TID_INT);
 db_set_value(hDB,0,"/Equipment/Galil/Variables/Acceleration",&acceleration,sizeof(acceleration),3,TID_INT);


   bk_init32(pevent);

   /* create SCLR bank */
   bk_create(pevent, "AXES", TID_INT, (void **)&pdata);
   for (int j=0;j<3;j++){
   *pdata++ = axes[j];
   }
   // *pdata++ = 456;
    ...
    bk_create(pevent, "SPID", TID_INT, (void **)&pdata);
    for (int j=0;j<3;j++){
      *pdata++ = speed[j];
    }
    bk_close(pevent,pdata);

    bk_create(pevent,"ACCL", TID_INT, (void **)&pdata);
    for(int j=0;j<3;j++){
